...
export const Skills = () => {
  ...
  return (
    <div className='flex w-full max-w-[1040px] mt-5 gap-5'>
      {categories.map((category, index) => (
        <SkillsWrapper key={index} title={category}>
          {
            skills.map((skill, index) => (
              (skill.name && skill.category === category) &&
              <div key={index} className='flex flex-col items-center sm:w-1/2 p-2 mb-5 transition ease-in-out hover:scale-105'>
                ...
                <div className='w-full bg-zinc-300 rounded-full shadow-md'>
                  <div className={`bg-zinc-500 text-white font-semibold text-sm text-center leading-none rounded-l-lg ${skill.percentage === 100 ? 'rounded-r-lg' : ''} py-1.5`} style={{ width: `${skill.percentage}%` }}></div>
                </div>
              </div>
            ))
          }
